  ...
  render(state, {
    ...
    onPreview: (pieceId, origin, source) => {
      ...
      state.hintText = state.pendingPlacement.valid
        ? `${piece?.name ?? 'Piece'} fits here. Rotate if you want, then Accept to place it.`
        : `${piece?.name ?? 'Piece'} does not fit here yet. Rotate or move it until the aura turns green.`;
      rerender();
    },
  ...
  document.getElementById('rotateBtn')?.addEventListener('click', () => {
    ...
    if (!id) {
      ...
      if (!rotatePiece(state.puzzle, id)) {
        ...
        state.hintText = state.pendingPlacement.valid
          ? 'That rotation fits. Accept to place it.'
          : 'That rotation does not fit here. Try another rotation or drag it elsewhere.';
      }
      if (piece && saved) {
        piece.x = saved.x;
        piece.y = saved.y;
