...
function setPuzzleDate(monthIndex: number, day: number, message: string) {
...
function rerender() {
  ...
  render(state, {
    ...
    onPreview: (pieceId, origin, source) => {
      ...
      state.hintText = state.pendingPlacement.valid
        ? `${piece?.name ?? 'Piece'} fits here. Rotate or Flip if you want, then Accept to place it.`
        : `${piece?.name ?? 'Piece'} does not fit here yet. Rotate, flip, or move it until the aura turns green.`;
      rerender();
    },
...
function bindEvents() {
  document.getElementById('resetBtn')?.addEventListener('click', () => { resetPuzzle(); rerender(); });
  document.getElementById('randomBtn')?.addEventListener('click', () => { setRandomDate(); rerender(); });
  document.getElementById('todayBtn')?.addEventListener('click', () => { setToday(); rerender(); });
  ...
  function transformSelectedPiece(transform: (pieceId: string) => boolean, verb: string): void {
    ...
    if (!id) {
      ...
      const saved = piece ? { x: piece.x, y: piece.y } : null;
      if (piece) {
        piece.x = null;
        piece.y = null;
      }
      if (!transform(id)) {
        ...
        state.hintText = state.pendingPlacement.valid
          ? `That ${verb} fits. Accept to place it.`
          : `That ${verb} does not fit here. Try another transform or drag it elsewhere.`;
      }
      if (piece && saved) {
        piece.x = saved.x;
        piece.y = saved.y;
  ...
  document.getElementById('dailyBtn')?.addEventListener('click', () => { setToday(); rerender(); });
